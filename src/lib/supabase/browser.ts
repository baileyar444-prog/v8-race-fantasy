...
const serverMock = {
  auth: {
    ...
    resetPasswordForEmail: async () => ({ data: null, error: new Error("Supabase client is only available in the browser.") }),
    updateUser: async () => ({ data: { user: null }, error: new Error("Supabase client is only available in the browser.") })
  },
  rpc: async () => ({ data: null, error: new Error("Supabase client is only available in the browser.") }),
  from: () => ({
    select: () => ({
      eq: () => ({
