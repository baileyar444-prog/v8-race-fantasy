        ...
        {selectedEvent ? (
          <div className="rounded-3xl border border-white/10 bg-black/20 p-3 sm:p-4">
            <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-start">
              <div>
                ...
                <p className="mt-1 text-track-muted">
                  {selectedScore?.status === "baseline"
                    ? `Baseline score: ${formatPoints(selectedScore.published_score)} pts added because no team was entered and no previous team was available.`
                    : selectedScore?.status === "carried_forward" || selectedTeam?.status === "carried_forward"
                      ? `Continued from ${carriedForwardSource(selectedTeam)}. Score: ${formatPoints(selectedScore?.published_score)} pts. Blank categories were scored as N/A.`
                      : selectedScore
                        ? `Published score: ${formatPoints(selectedScore.published_score)} pts`
                        : selectedTeam
                          ? "Team saved. Waiting for Race Control to publish scores."
                          : "No team selected for this event yet."}
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {selectedEvent.is_open_event && !isLocked(selectedEvent.lockout_at, selectedEvent.manual_lock, now) ? (
                  ...
                  <span className="pill">{selectedScore?.status === "baseline" ? "Baseline applied" : selectedTeam?.status === "carried_forward" ? "Continued team" : "No selection"}</span>
                )}
              </div>
            </div>
            ...
            <div className="mt-5 space-y-2">
              {selectedPickScores.length ? categories.map((category) => {
                const pick = selectedPickScores.find((item) => item.category === category);
                ...
                categories.map((category) => {
                  const pick = selectedTeam.fantasy_team_picks?.find((item) => item.category === category);
