    ...
    <div className="card" style={{ background: profile.banner_colour ? `linear-gradient(135deg, ${profile.banner_colour}22, rgba(17,24,39,.86))` : undefined }}>
      <div className="grid gap-4 xl:grid-cols-[1fr_300px] xl:items-start">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:min-w-[520px]">
            <Link className="rounded-xl bg-track-orange px-3 py-3 text-center text-sm font-black text-black shadow-glow hover:bg-orange-300 sm:rounded-2xl sm:px-4" href="/pick-team">Edit team</Link>
            <Link className="rounded-xl border border-white/10 bg-white/5 px-3 py-3 text-center text-sm font-black hover:bg-white/10 sm:rounded-2xl sm:px-4" href="/team-history">My Team</Link>
      ...
      <div className="mt-4 grid gap-2 sm:gap-3 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-3 sm:rounded-2xl sm:p-4">
          ...
          <div className="mt-1 text-2xl font-black sm:text-3xl">{formatRank(rankStats.rank)}</div>
      ...
      <section className="mt-5 hidden rounded-3xl border border-white/10 bg-black/20 p-4 sm:block">
            ...
            {popularDrivers.map((driver, index) => (
              <div key={driver.driverId} className="rounded-2xl border border-white/10 bg-white/5 p-3">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-center text-[11px] font-black">
                  <div className="rounded-xl bg-black/20 p-2"><div>{driver.ownership}%</div><div className="text-track-muted">Picked</div></div>
                  <div className="rounded-xl bg-black/20 p-2"><div>{driver.captaincy}%</div><div className="text-track-muted">C</div></div>
                  <div className="rounded-xl bg-black/20 p-2"><div>{driver.viceCaptaincy}%</div><div className="text-track-muted">VC</div></div>
                </div>
              </div>
            ))}
          </div>
        ...
        {captainFavourite ? (
          <div className="mt-3 rounded-2xl border border-track-orange/25 bg-track-orange/10 p-3 text-sm">
            ...
            #{captainFavourite.carNumber} {captainFavourite.driverName} is on {captainFavourite.captaincy}% captaincy.
          </div>
        ) : null}
      </section>
