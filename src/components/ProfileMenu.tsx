...
export function ProfileMenu() {
  ...
  return (
    <div className="group relative">
      <button className="flex items-center gap-2 rounded-full border border-white/10 px-2 py-1.5 sm:gap-3 sm:px-3 sm:py-2" style={{ background: profile.banner_colour ? `linear-gradient(135deg, ${profile.banner_colour}33, rgba(255,255,255,.05))` : undefined }}>
        <Shield
      ...
      <div className="invisible absolute right-0 top-full z-50 mt-2 max-h-[70vh] w-[calc(100vw-1.5rem)] max-w-72 overflow-y-auto rounded-2xl border border-white/10 bg-[#101827] p-2 opacity-0 shadow-2xl transition group-hover:visible group-hover:opacity-100 sm:w-64">
        ...
        <Link href="/share-team" className="block rounded-xl px-3 py-2 text-sm font-bold hover:bg-white/10">
          Share team card
        </Link>
        <Link href="/story-assets" className="block rounded-xl px-3 py-2 text-sm font-bold hover:bg-white/10">
          Story assets
        </Link>
        <Link href="/round-preview" className="block rounded-xl px-3 py-2 text-sm font-bold hover:bg-white/10">
          Round preview
        </Link>
        ...
        {profile.role === "admin" ? (
          <>
            ...
            <Link href="/member-emails" className="block rounded-xl px-3 py-2 text-sm font-black text-orange-100 hover:bg-track-orange/10">
              Member emails
            </Link>
          </>
        ) : null}
